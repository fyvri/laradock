...
usage() {
    ...
    echo "      --no-deps       Don't restart dependent services"
...
if [[ $NO_INTERACTION == false && $IS_DRY_RUN != true ]]; then
    read -p "Are you sure you want to reboot all services? [Y/n]: " CONFIRMATION
...
$MAIN_DOCKER_COMPOSE_COMMAND ls --all | grep "/${MAIN_DIRECTORY}/${MAIN_COMPOSE_FILE}" | awk '{print $1}' | while IFS= read -r PROJECT_NAME; do
    $MAIN_DOCKER_COMPOSE_COMMAND $MAIN_DOCKER_COMPOSE_OPTIONS ${PROJECT_NAME:+--project-name $PROJECT_NAME} restart $@
    echo ""
done