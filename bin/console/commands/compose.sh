...
DOCKER_OPTIONS=$(echo "$(${MAIN_DOCKER_COMPOSE_COMMAND} up --help | grep -v -e "--detach" -e "--build")" | sed -n '/Options:/,/^$/ { /Options:/d; p }')
OPTIONS=""
...
$MAIN_DOCKER_COMPOSE_COMMAND \
    ...
    up --build --detach $OPTIONS ${IS_DRY_RUN:+"--dry-run"}
$MAIN_DOCKER_COMMAND image prune -f > /dev/null 2>&1 || true